...
- The gift-feedback drawer (the sheet that slides up when you tap a gift idea's options) now matches the finalized design: the feedback options are a clean text list with "Keep this in the mix" emphasized at the top (the placeholder dot in front of each option is gone), the rows sit on a tighter line spacing, and the "Send" control on the "Gift feedback" note screen is now a gold outlined up-arrow instead of a solid paper-plane button (DEV-190).
