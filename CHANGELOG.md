...
- The small icon buttons that were fiddly to tap now have a larger touch area meeting Apple's recommended minimum, so they're easier to hit accurately: the bug-report icon and avatar in the top header, the back arrows on the Settings sub-screens (Support, Gifting, Notifications, Profile, Billing) and FAQ, the back/close buttons in the Add-a-person conversation and recipient detail dialogs, and the "…" options buttons on Moments, gift, and occasion cards. The icons look exactly the same — only the tappable region grew (DEV-204).
