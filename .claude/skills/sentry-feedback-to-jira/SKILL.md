...
- **Dedup check (mandatory, in order):**
  ...
  4. **Fixed since the report:** the three checks above only see open tickets, so a report about a bug fixed days later still looks new — and feedback can be weeks old by the time it's swept. Run this on every item that survives 1–3. The cutoff is the **report date**, not the reporter's build: binaries pick up later JS through OTAs, so anything that shipped before the report was probably already on the reporter's phone and evidently didn't fix it.
     - **Look for a fix merged after the report.** `.claude/scripts/jira-api search 'project = DEV AND statusCategory = Done AND resolved >= "<report date>" AND text ~ "<keywords>"' 'summary,status,resolutiondate' 10` (this also covers fixes in `be-gifted`), and `git log --first-parent --since="<report date>" --format='%h %cs %s' origin/main -- <files>` with the files located above — plain paths, `:line` stripped (a `path:line` pathspec silently matches nothing). Merge subjects carry ticket keys. A Done ticket or commit that addresses the same symptom on the same screen is a **likely fix**; read it, don't match on keywords alone.
...
FEEDBACK RUN <date>
  ...
  verify:     DEV-xxx <summary> (likely fixed by DEV-yyy, merged after the report — verify-first)
