...
- **Dedup check (mandatory, in order):**
  ...
  4. **Fixed since the report:** the three checks above only see open tickets, so a report about a bug fixed days later still looks new — and feedback can be weeks old by the time it's swept. Run this on every item that survives 1–3. The cutoff is the **last production OTA before the report**, not the reporter's build: binaries pick up later JS through OTAs, so anything shipped before that OTA was probably already on the reporter's phone, while anything merged after it — even before the report date — hadn't reached them yet. Find it with `eas update:list --branch production --limit 50 --non-interactive --json` (newest first; the entry's `message` starts with its date) — the newest update dated on or before the report whose `runtimeVersion` matches the reporter's binary (its `appVersion`). If that fails, use the report date minus 14 days.
     - **Look for a fix merged after the cutoff.** `.claude/scripts/jira-api search 'project = DEV AND statusCategory = Done AND resolved >= "<cutoff>" AND text ~ "<keywords>"' 'summary,status,resolutiondate' 10` (this also covers fixes in `be-gifted`), and `git log --first-parent --since="<cutoff>" --format='%h %cs %s' origin/main -- <files>` with the files located above — plain paths, `:line` stripped (a `path:line` pathspec silently matches nothing). Merge subjects carry ticket keys. A Done ticket or commit that addresses the same symptom on the same screen is a **likely fix**; read it, don't match on keywords alone.
     - **Record the reporter's binary** for whoever verifies: from `eas build:list --platform ios --limit 50 --non-interactive --json`, the entry with matching `appBuildVersion`, `status == "FINISHED"` and `distribution == "STORE"` (cancelled and development builds reuse numbers — build 60 has a cancelled dev entry from 2026-09-10 next to the shipped f59d5c9). Sentry items: the event's `release`/`dist` tags give the build number. Put it in the body — `Reported … build 60 (f59d5c9) …` — or `build 60 (commit unknown)` when `eas` fails or nothing matches.
     - A likely fix doesn't suppress the item — the reporter may have hit a variant the fix missed — but it changes how it's filed (Steps 3–4).
...
FEEDBACK RUN <date>
  ...
  verify:     DEV-xxx <summary> (likely fixed by DEV-yyy, merged after the reporter's last OTA — verify-first)
