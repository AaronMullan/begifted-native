...
6. **Boot the app and screenshot the _whole_ screen.** Use the `run` skill (or `npm run ios`) to launch the simulator, navigate to `<app-route>` (deep link or tap through), then:
   ...
   **Open every overlay the screen owns and screenshot each one.** Dialogs, menus, dropdowns, and bottom sheets never appear in an at-rest route screenshot, so a screens-only sweep silently skips them — this is exactly how a fleet of white-surfaced modals shipped against a beigeLight spec (DEV-350). Trigger each overlay (tap the button, open the menu), capture it, and compare its surface color, radius, and contents against the design's overlay state. Paper overlays inherit their background from the theme's `elevation` palette (`app/_layout.tsx`), not per-component styles — if an overlay's color is wrong, check the theme binding before hunting in the component.
