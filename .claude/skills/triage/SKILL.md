---
name: triage
description: Triage swarm — pull top Sentry error clusters (and any pasted PM feedback), root-cause each in parallel against the codebase, dedup against existing Jira, and produce a prioritized table of draft tickets for review before filing. Use when clearing a Sentry/feedback backlog.
---

# Triage Swarm

Turn the Sentry error backlog (plus optional pasted PM feedback) into deduplicated, root-caused, prioritized Jira tickets — investigated in parallel, filed only after review.
...
3. If the user pasted PM feedback, add each distinct item to the work-list alongside the Sentry clusters. TestFlight's native feedback (screenshot + crash submissions) does not need pasting: `/sentry-feedback-to-jira` reads it from the App Store Connect API (`.claude/scripts/asc-api screenshots|crashes`) every nightly run and files it as `user-feedback` tickets — pull those keys into the work-list rather than re-reading the submissions here.
