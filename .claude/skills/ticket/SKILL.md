...
1. **Read the ticket.** Fetch it via REST (not the Jira MCP — REST + `jq` keeps the payload small; see _Jira reads vs writes_ below):
   ...
   **`verify-first` tickets: reproduce before coding.** The feedback sweep adds this label when a fix for the same symptom merged after the report (the body names it on a `Likely fixed by:` line). Try to reproduce the bug on current `main` first (`/drive-sim`). If it reproduces, carry on with step 2 — the named fix missed a case. If it doesn't:
   - Confirm the named fix is **live** — its merge commit is in a production OTA (`eas update:view <group> --json` → `gitCommitHash`, then `git merge-base --is-ancestor`) or in a released store build. Merged-but-unshipped is not fixed for testers.
