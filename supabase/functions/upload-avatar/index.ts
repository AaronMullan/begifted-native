...
// Mirrors upload-recipient-photo: direct client uploads to the storage buckets
// get RLS-rejected on this project even with permissive policies, so we upload
// with service_role after verifying the caller's JWT and constrain the object
// path to the caller's own user_id — security equivalent to path-scoped RLS.
...
serve(async (req) => {
  ...
  try {
    ...
    return internalErrorResponse("upload-avatar", err, corsHeaders);
  }
});
